...
import { reportingLeave } from '../../../actions/reporting';

import Criteria from './criteria';
import Chart from './chart';
import { formatCriteria } from './tools';

const useConnect = ({ refreshAction }) => {
...
const GroupByPeriod = ({ refreshAction, initialCriteria, initialDisplay, additionalCriteriaFactory, amountSelectorFactory, periodKey }) => {
  const [criteria, setCriteria] = useState(initialCriteria);
  const [display, setDisplay] = useState(initialDisplay);

  const { refresh, leave, data } = useConnect({ refreshAction });
